...
impl UniversalExpertSystem {
    ...
    fn explain_refusal(
        ...
        // Return structured markers - decoder generates appropriate text from learned patterns
        // NO hardcoded responses - all text is generated by the model
        let reason = if !verification.verified {
            format!(
                "UNVERIFIED:LOW_CONFIDENCE:{}",
                verification.reasoning
            )
        } else if !formal_verification.verified {
            format!(
                "UNVERIFIED:FORMAL_MISMATCH:{}",
                formal_verification.reasoning
            )
        } else {
            "UNVERIFIED:UNKNOWN".to_string()
        };
